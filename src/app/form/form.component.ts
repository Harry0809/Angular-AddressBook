...
export class FormComponent implements OnInit {
  ...
  addContact= new FormGroup({
    Name : new FormControl('',[Validators.pattern("^[A-Z]{1}[a-z]{2,}[\\s][A-Z]{1}[a-z]{2,}$")]),
  ...
  get fullName() {
    return this.addContact.get('Name');
  }
  ...
  getContactById(){
    if(this.Id==null){
      this.addContact;
      return
    }else{    this.contactService.getContactById(this.Id).subscribe((response:any) => {
      console.log(response.content);
      this.addContact = new FormGroup({
        Name : new FormControl(response.content['Name']),
